...
void Slave_Complete_Callback(uint8_t *rx_data, uint16_t len)
{
    ...
    if (len == 1) {
        if ((rx_data[0] >= UID_REG_ADDR_START) && (rx_data[0] <= UID_REG_ADDR_END)) {
            ...
            tx_prepared = 1;
        } else if (rx_data[0] == DEVICE_ID_REG_ADDR) {
            tx_buf[0] = DEVICE_ID;
            i2c1_set_send_data((uint8_t *)&tx_buf[0], 1);
            tx_prepared = 1;
        }
        if ((rx_data[0] >= FIRMWARE_VERSION_REG_ADDR) && (rx_data[0] <= I2C_ADDRESS_REG_ADDR)) {
            tx_buf[0] = FIRMWARE_VERSION;
            tx_buf[1] = i2c_addr_reg;
            i2c1_set_send_data((uint8_t *)&tx_buf[rx_data[0] - FIRMWARE_VERSION_REG_ADDR],
                               I2C_ADDRESS_REG_ADDR - rx_data[0] + 1);
            tx_prepared = 1;
        }
    } else if (len > 1) {
        if (rx_data[0] == IAP_UPDATE_REG_ADDR && len == 2) {
            if (rx_data[1]) {
                NVIC_SystemReset();
            }
