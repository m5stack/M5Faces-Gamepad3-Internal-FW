...
{
    uint64_t temp = flash_read_double_word(STM32F0XX_FLASH_PAGE15_ADDR);
    set_byte_in_uint64(&temp, I2C_ADDR_FLASH_OFFSET, data);

    __disable_irq();
    bool erased  = flash_erase_page(STM32F0XX_FLASH_PAGE15_ADDR);
    bool written = false;

    if (erased) {
        written = flash_write_double_word(STM32F0XX_FLASH_PAGE15_ADDR, temp);
    }
    __enable_irq();
