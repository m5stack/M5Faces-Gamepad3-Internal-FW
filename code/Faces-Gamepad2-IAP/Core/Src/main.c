...
#define FW_LENGTH           (((uint32_t)0x2C00))
#define FW_CRC_ADDR         (APPLICATION_ADDRESS + FW_LENGTH - 4)
...
/* USER CODE BEGIN PV */
__IO uint16_t I2C_Receive_Counter = 0;
__IO uint8_t Receive_Buffer[1032];
__IO EventStatus i2c_event = NOEVENT;
__IO uint16_t error;
__IO uint32_t NbrOfPage    = 0x00;
__IO uint32_t EraseCounter = 0x00, Address = 0x00;
__IO FLASH_Status FLASHStatus       = FLASH_COMPLETE;
__IO TestStatus MemoryProgramStatus = PASSED;
__IO uint32_t JumpAddress;
__IO pFunction JumpToApplication;
__IO uint8_t opcode;
__IO uint32_t lastTime = 0;
...
int main(void)
{
    ...
    iap_gpio_init();
    LL_mDelay(30);  // 等待稳定
    if (LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_9) == 0 && LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_10) == 0) {
